...
router.get("/", async (_req: Request, res: Response) => {
    try
    {
        ...
        festivalRows.forEach((festival) => {
            const logoFiles = fs.readdirSync(FESTIVAL_LOGO_DIR).filter((f: string) => f.startsWith(`${festival.id}.`));
...
router.get("/:id", async (req: Request, res: Response) => {
    ...
    try
    {
        const {rows} = await pool.query<Festival>("SELECT * FROM festivals WHERE id = $1", [id]);
        if (rows.length === 0)
        {
        ...
        {
            const {rows: gameZoneRows} =
                await pool.query<Game_Zone>("SELECT * FROM game_zone WHERE tarif_zone_id = $1", [tarifZone.id]);
            tarifZone.game_zones = gameZoneRows;
        }

        // Check for logo
        const logoFiles = fs.readdirSync(FESTIVAL_LOGO_DIR).filter((f: string) => f.startsWith(`${id}.`));
...
router.post("/:id/tarif-zones", requireAdmin, async (req: Request, res: Response) => {
    ...
    try
    {
        ...
        const {rows: festivalRows} = await pool.query("SELECT id FROM festivals WHERE id = $1", [id]);
...
router.delete("/:id/tarif-zones/:tarifZoneId", requireAdmin, async (req: Request, res: Response) => {
    ...
    try
    {
        const {rowCount} = await pool.query("DELETE FROM tarif_zone WHERE id = $1", [tarifZoneId]);
        if (rowCount === 0)
        {
...
router.post("/:id/tarif-zones/:tarifZoneId/game-zones", requireAdmin, async (req: Request, res: Response) => {
    ...
    try
    {
        ...
        const {rows: tarifZoneRows} = await pool.query("SELECT id FROM tarif_zone WHERE id = $1", [tarifZoneId]);
...
router.delete(
    "/:id/tarif-zones/:tarifZoneId/game-zones/:gameZoneId", requireAdmin, async (req: Request, res: Response) => {
        ...
        try
        {
            const {rowCount} = await pool.query("DELETE FROM game_zone WHERE id = $1", [gameZoneId]);
            if (rowCount === 0)
            {
...
router.post("/:id/logo", requireAdmin, upload.single("logo"), async (req: Request, res: Response) => {
    ...
    const {rows} = await pool.query("SELECT id FROM festivals WHERE id = $1", [id]);
    if (rows.length === 0)
    {
    ...
    const existingFiles = fs.readdirSync(FESTIVAL_LOGO_DIR).filter((f: string) => f.startsWith(`${id}.`));

    existingFiles.forEach((f: string) => fs.unlinkSync(`${FESTIVAL_LOGO_DIR}/${f}`));
