...
export class FestivalService
{
     ...
     */
    updateFestival(festivalId: number, festivalData: Partial<FestivalDto>, logoFile?: File, deleteLogo?: boolean): void
    {
        this.http
            ...
            .pipe(
                tap((response) => {
                    ...
                    {
                        this.festivals.update(
                            (festivals) => festivals.map((f) => (f.id === festivalId ? {...f, ...response} : f)));
                    }
        ...
        this.http
            ...
            .pipe(
                tap((response) => {
                    ...
                    this.festivals.update(
                        (festivals) => festivals.map((f) => (f.id === festivalId ? {...f, logoUrl: fullUrl} : f)));

                    // Update current festival if applicable
                    if (this.currentFestival()?.id === festivalId)
                    {
                        this.currentFestival.update((festival) => (festival ? {...festival, logoUrl: fullUrl} : null));
                    }
                }),
                catchError((err) => {
        ...
        this.http
            ...
            .pipe(
                tap(() => {
                    ...
                    // Update in festivals list
                    this.festivals.update(
                        (festivals) => festivals.map((f) => (f.id === festivalId ? {...f, logoUrl: undefined} : f)));

                    // Update current festival if applicable
                    if (this.currentFestival()?.id === festivalId)
                    {
                        this.currentFestival.update(
                            (festival) => (festival ? {...festival, logoUrl: undefined} : null));
                    }
                }),
                catchError((err) => {
    ...
    addTarifZone(festivalId: number, zoneData: Omit<ZoneTarifDTO, 'id'>): void
    {
        this.http
            .post<ZoneTarifDTO>(
                `${environment.apiUrl}/festivals/${festivalId}/tarif-zones`, zoneData, {withCredentials: true})
            .pipe(
                tap((response) => {
    ...
    {
        this.http
            .put<ZoneTarifDTO>(`${environment.apiUrl}/festivals/${festivalId}/tarif-zones/${tarifZoneId}`, zoneData, {
                withCredentials: true,
            })
            .pipe(
                tap((response) => {
    ...
    removeTarifZone(festivalId: number, tarifZoneId: number): void
    {
        this.http
            .delete<void>(
                `${environment.apiUrl}/festivals/${festivalId}/tarif-zones/${tarifZoneId}`, {withCredentials: true})
            .pipe(
                tap(() => {
     ...
     */
    addGameZone(festivalId: number, tarifZoneId: number, gameZoneData: Omit<ZoneGameDTO, 'id'>): void
    {
        this.http
            .post<ZoneGameDTO>(
     ...
     * Update a game zone
     */
    updateGameZone(festivalId: number, tarifZoneId: number, gameZoneId: number, gameZoneData: Partial<ZoneGameDTO>):
        void
    {
        this.http
            .put<ZoneGameDTO>(
